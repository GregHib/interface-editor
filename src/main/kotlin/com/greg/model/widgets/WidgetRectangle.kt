...
class WidgetRectangle(builder: WidgetBuilder, id: Int) : Widget(builder, id) {
    ...
    fun fillProperty(): ObjectProperty<Color> {
        if (fill == null)
            fill = SimpleObjectProperty<Color>(this, "fill", Color.BLUE/*Settings.getColour(SettingsKey.DEFAULT_RECTANGLE_FILL_COLOUR)*/)
    ...
    fun strokeProperty(): ObjectProperty<Color> {
        if (stroke == null)
            stroke = SimpleObjectProperty<Color>(this, "stroke", Color.PURPLE/*Settings.getColour(SettingsKey.DEFAULT_RECTANGLE_STROKE_COLOUR)*/)
